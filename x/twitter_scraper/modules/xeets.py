...
def get_tweets(query, pages=25):
    ...
    if query.startswith("#"):
        query = quote(query)
        url = f"https://x.com/i/search/timeline?f=tweets&vertical=default&q={query}&src=tyah&reset_error_state=false&"
    else:
        url = f"https://x.com/i/profiles/show/{query}/timeline/tweets?"
    url += after_part

    headers = {
        "Accept": "application/json, text/javascript, */*; q=0.01",
        "Referer": f"https://x.com/{query}",
        ...
        while pages > 0:
            ...
            for tweet, profile in zip(
                ...
                hashtags = [
                    ...
                    for hashtag_node in tweet.find(".x-hashtag")
                ]

                urls = [
                    url_node.attrs["data-expanded-url"]
                    for url_node in (
                        tweet.find("a.x-timeline-link:not(.u-hidden)") +
            ...
            for tweet in tweets:
                ...
                tweet["text"] = re.sub(
                    r"(\S)pic\.x", "\g<1> pic.x", tweet["text"], 1
                )
                yield tweet
...
# for searching:
#
# https://x.com/i/search/timeline?vertical=default&q=foof&src=typd&composed_count=0&include_available_features=1&include_entities=1&include_new_items_bar=true&interval=30000&latent_count=0
